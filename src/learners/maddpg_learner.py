...
class MADDPGLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # Train the actor
        h = self.mac.init_hidden(batch_size)
        pis = []
        actions = []
        for t in range(batch.max_seq_length-1):
            pi, h = self.mac.forward(batch, t=t).view(batch_size, 1, self.n_agents, -1)
            # pi, h = self.mac.forward(batch, h, t=t).view(batch_size, 1, self.n_agents, -1)

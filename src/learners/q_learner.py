...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        mac_out = []
        h = self.mac.init_hidden(batch.batch_size)
        for t in range(batch.max_seq_length):
            # agent_outs, h = self.mac.forward(batch, h, t=t)
        ...
        target_mac_out = []
        h = self.target_mac.init_hidden(batch.batch_size)
        for t in range(batch.max_seq_length):
            target_agent_outs, h = self.target_mac.forward(batch, t=t)
            target_mac_out.append(target_agent_outs)

        # We don't need the first timesteps Q-Value estimate for calculating targets
        target_mac_out = th.cat(target_mac_out[1:], dim=1)  # Concat across time
